...
An asset that exposes a named list of integer values configurable in the editor, usable as a designer-facing data table for integer parameters.

**Intent:** Allow designers to define a set of named integer values (e.g. score thresholds, tile counts) in the editor without code changes, then reference them by asset GUID from components.

**Use-case:** Create a `SmartIntAsset` with matching `Titles` and `Values` arrays. Reference the asset by GUID from a component or system, then read the desired value by index at runtime.
...
Display names for each integer value, displayed in the editor alongside the corresponding entry in `Values`.
...
The integer values paired with the corresponding `Titles` entries.
