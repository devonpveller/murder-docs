...
An asset that exposes a named list of float values configurable in the editor, usable as a designer-facing data table for numeric parameters.

**Intent:** Allow designers to define a set of named float values (e.g. difficulty multipliers, speed tiers) in the editor without code changes, then reference them by asset GUID from components.

**Use-case:** Create a `SmartFloatAsset` with matching `Titles` and `Values` arrays. Reference the asset by GUID from a component or system, then read the desired value by index at runtime.
...
Display names for each float value, displayed in the editor alongside the corresponding entry in `Values`.
...
The float values paired with the corresponding `Titles` entries.
