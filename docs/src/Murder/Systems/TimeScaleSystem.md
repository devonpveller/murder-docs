...
Records each newly added entity's `TimeScaleComponent.Value` in an internal per-entity dictionary and recalculates the aggregated global time scale.
...
Updates the stored multiplier for each modified entity and recalculates the aggregated global time scale.
...
Removes the time-scale multiplier entry for each removed entity and recalculates the aggregated global time scale.
