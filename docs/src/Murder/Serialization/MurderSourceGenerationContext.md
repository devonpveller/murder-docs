...
**Intent:** The AOT- and trimming-compatible `JsonSerializerContext` that the `Murder.Serializer` Roslyn generator emits at compile time for every type the engine needs to (de)serialize. It is the empty partial class declared by `Murder.Serializer`'s `Templates.GenerateContext` (`public partial class MurderSourceGenerationContext : JsonSerializerContext, IMurderSerializer`); its other half — every `[JsonSerializable(typeof(...))]`-attributed property below — is filled in by re-running `System.Text.Json`'s own source generator against that partial declaration, so this context ends up with one `JsonTypeInfo<T>` property per registered type, generated without any hand-written code.

**Use-case:** Consumed by [MurderSerializerOptionsExtensions.Options](../../Murder/Serialization/MurderSerializerOptionsExtensions.html), which points its `TypeInfoResolver` at `MurderSourceGenerationContext.Default` (combined with a downstream game project's own generated context, if any). Game code essentially never references this class directly — go through `Game.Data.SerializationOptions` or [FileManager](../../Murder/Serialization/FileManager.html) instead — but it is where every Murder component, asset, interaction, state machine, and their private serialized fields get their compile-time JSON contract.
...
Creates a context using the default `JsonSerializerOptions`. Generated by `System.Text.Json`'s own source generator (not hand-written); most code should use the cached `Default` singleton property instead of constructing a new instance.

```csharp
public MurderSourceGenerationContext(JsonSerializerOptions options)
```

Creates a context that will use `options` (rather than the default options) when its `JsonTypeInfo<T>` properties need to resolve converters/settings not baked in at compile time.
...
The lazily-created singleton instance of this context, generated by the `System.Text.Json` source generator. This is what [MurderSerializerOptionsExtensions.Options](../../Murder/Serialization/MurderSerializerOptionsExtensions.html) points the `TypeInfoResolver` at (combined with the parent assembly's context, if any) so that every `[JsonSerializable]`-registered Murder type resolves without falling back to slow, trimming-unsafe reflection.
...
```csharp
public JsonTypeInfo<T> DialogueId { get; }
```
...
#### ImmutableDictionaryDialogueIdLineInfo

```csharp
public JsonTypeInfo<T> ImmutableDictionaryDialogueIdLineInfo { get; }
```
...
#### InteractiveComponentPauseEventLayerInteraction

```csharp
public JsonTypeInfo<T> InteractiveComponentPauseEventLayerInteraction { get; }
```
...
#### InteractiveComponentPlayEventInteraction

```csharp
public JsonTypeInfo<T> InteractiveComponentPlayEventInteraction { get; }
```
...
#### InteractiveComponentPlayEventWithSettingsInteraction

```csharp
public JsonTypeInfo<T> InteractiveComponentPlayEventWithSettingsInteraction { get; }
```
...
#### InteractiveComponentSetSoundParameterOnInteraction

```csharp
public JsonTypeInfo<T> InteractiveComponentSetSoundParameterOnInteraction { get; }
```
...
#### InteractiveComponentStopEventInteraction

```csharp
public JsonTypeInfo<T> InteractiveComponentStopEventInteraction { get; }
```
...
#### PauseEventLayerInteraction

```csharp
public JsonTypeInfo<T> PauseEventLayerInteraction { get; }
```
...
#### PlayEventInteraction

```csharp
public JsonTypeInfo<T> PlayEventInteraction { get; }
```
...
#### PlayEventWithSettingsInteraction

```csharp
public JsonTypeInfo<T> PlayEventWithSettingsInteraction { get; }
```
...
#### SetSoundParameterOnInteraction

```csharp
public JsonTypeInfo<T> SetSoundParameterOnInteraction { get; }
```
...
#### StopEventInteraction

```csharp
public JsonTypeInfo<T> StopEventInteraction { get; }
```
