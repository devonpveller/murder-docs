...
`MurderEntityExtensions` is compile-time generated by `Bang.Generator` (see `bang\src\Bang.Generator\Templating\Templates.EntityExtensions.cs`), which scans the project for every `IComponent` and `IMessage` type and emits a matching set of strongly-typed `Entity` extension methods here — thousands of small, mechanical, near-identical entries, one group per component/message. There is no need to read through them individually: once you know the naming convention below, the method you want for any given component or message is entirely predictable from its type name.

For a component named `FooComponent` (documented with friendly name `Foo`), the generator emits six extension methods, each a thin, allocation-free wrapper around the matching index-based [Entity](../../Bang/Entities/Entity.html) method (using the component's fixed index from [MurderComponentTypes](../../Bang/Entities/MurderComponentTypes.html), so no `Type` lookup or dictionary hashing is needed at the call site):
- `bool HasFoo(this Entity e)` — whether the entity currently has the component; wraps `Entity.HasComponent(int)`.
- `FooComponent GetFoo(this Entity e)` — fetch the component, asserting if it is missing; wraps `Entity.GetComponent<T>(int)`.
- `FooComponent? TryGetFoo(this Entity e)` — fetch the component, or `null` if missing, without asserting; implemented as `e.HasFoo() ? e.GetFoo() : null`.
- `void SetFoo(this Entity e, FooComponent component)` — add or replace the component; wraps `Entity.AddOrReplaceComponent<T>(T, int)`. If `FooComponent` has one or more public constructors, an additional `SetFoo(this Entity e, ...)` overload is generated per constructor, taking the constructor's parameters directly and building the component for you (e.g. `entity.SetAlpha(0.5f)` instead of `entity.SetAlpha(new AlphaComponent(0.5f))`).
- `Entity WithFoo(this Entity e, FooComponent component)` — identical to `SetFoo`, but returns the entity so calls can be chained while building up an entity (e.g. `world.AddEntity().WithFoo(...).WithBar(...)`).
- `bool RemoveFoo(this Entity e)` — remove the component, returning whether it was present; wraps `Entity.RemoveComponent(int)`.

For a message named `FooMessage`, the pattern is narrower since messages are transient, fire-and-forget signals rather than persistent state (see [Entity](../../Bang/Entities/Entity.html) for the messaging model), so there is no `Get`/`TryGet`/`Set`/`With`:
- `bool HasFooMessage(this Entity e)` — whether the entity has a pending message of this type this frame; wraps `Entity.HasMessage(int)`.
- `void SendFooMessage(this Entity e, ...)` — send the message, notifying any subscriber; wraps `Entity.SendMessage(int, T)`. Like `Set`, this gets one overload per public constructor of `FooMessage`, plus a parameterless overload if the message type itself has no fields.
- `bool RemoveFooMessage(this Entity e)` — remove a pending message before the frame ends; wraps `Entity.RemoveMessage(int)`.

As an agent or developer working in this codebase, always prefer these generated methods over the generic `Entity` component/message APIs when the concrete type is known at the call site: call `entity.GetPosition()`/`entity.HasAgent()`/`entity.SetAlpha(0.5f)` instead of `entity.GetComponent<PositionComponent>()`/`entity.HasComponent<AgentComponent>()`/`entity.AddOrReplaceComponent(new AlphaComponent(0.5f))`. They are strongly typed (catching typos and type mismatches at compile time), avoid the `Type`-based lookup path, and read closer to the domain (e.g. `entity.RemoveDestroyAtTime()`). Do not hand-edit this file or any individual entry in it — it is fully regenerated from the project's component/message declarations on every build; add or change fields on the component/message type itself instead.
