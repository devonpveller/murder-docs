...
`MurderComponentTypes` is compile-time generated by `Bang.Generator` (see `bang\src\Bang.Generator\Templating\Templates.ComponentTypes.cs`), which scans the project for every type implementing `IComponent` and emits one `public static int` property per component here, named after the component (with the `Component` suffix stripped — e.g. `AgentComponent` becomes `Agent`). Each property returns a unique integer index, computed as an offset from `MurderComponentsLookup.MurderNextLookupId` plus the component's position in the generator's ordering; components that are transform-like get pinned to the shared `BangComponentTypes.Position` index instead (see [BangComponentTypes](../../Bang/Entities/BangComponentTypes.html)). These indices are exactly what [Entity](../../Bang/Entities/Entity.html) uses internally for its index-based `HasComponent(int)`/`GetComponent<T>(int)`/`AddComponent<T>(T, int)` overloads, and what [ComponentsLookup](../../Bang/ComponentsLookup.html) maps `Type → int` and back through.

You will rarely need to reference `MurderComponentTypes.*` directly: it exists mainly as generated plumbing consumed by [MurderEntityExtensions](../../Bang/Entities/MurderEntityExtensions.html) (whose `Get`/`Has`/`TryGet`/`Set`/`With`/`Remove` methods pass the right constant for you) and by engine-internal code that needs to bulk-iterate or reason about component indices (e.g. serialization, the editor, or systems that filter entities by a bitset of indices). Prefer the strongly-typed extension methods on `Entity` (e.g. `entity.HasAgent()`, `entity.GetAlpha()`) over comparing against a raw index from this class by hand; reach for `MurderComponentTypes.SomeComponent` only when you specifically need the numeric id itself (for example, when building a custom index/bitset-based filter).
